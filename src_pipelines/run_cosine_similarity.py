...
def get_w2v_model(readdir, writedir, file_name):
    text_filename = os.path.join(readdir, "%s.txt" % file_name) #file containing tweet text
    model_filename = os.path.join(writedir, "%s_300.bin" % file_name) #model file
...
def loop_cs_once(readdir, writedir, file_name):
    model = get_w2v_model(readdir, writedir, file_name)
...
if __name__ == '__main__':
    readdir, writedir, file_name = "", "", "" #TODO add readdir, writedir, and file name
    loop_cs_once()